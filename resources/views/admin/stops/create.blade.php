...
@section('content')
<h1 class="text-2xl text-center font-bold mb-6">Adaugă oprire pentru ruta '{{ $route->route_tur }} - {{ $route->route_retur }}'---Ora:{{ $route->start_time }}</h1>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white">
        <form action="{{ route('admin.stops.store', $route) }}" method="POST">
            @csrf
            <div class="grid gap-6 mb-6 md:grid-cols-2 mx-4 mt-6">
                <div>
                    ...
                    <select id="route_stop" name="route_stop" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('route_stop') border-red-500 @enderror" required>
                    ...
                    <input type="text" id="pickup" name="pickup" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('pickup') border-red-500 @enderror" placeholder="MOL Centura" value="{{ old('pickup') }}" required />
                    @error('pickup')
                        <p class="text-red-500 text-xs mt-1 rounded-md">{{ $message }}</p>
                    @enderror
                </div> 
                <div class="w-full max-w-xs">
                    <label for="stop_time" class="block text-lg font-medium text-gray-700">Alege Ora( cand ajunge la oprire )</label>
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <input type="text" id="stop_time" name="stop_time" class="timepicker block w-full pl-3 pr-10 sm:text-sm border-gray-300 rounded-md @error('time') border-red-500 @enderror" value="{{ old('stop_time') }}" required>
                        @error('stop_time')
                            <p class="text-red-500 text-xs mt-1 rounded-md">{{ $message }}</p>
                        @enderror
                    </div>
