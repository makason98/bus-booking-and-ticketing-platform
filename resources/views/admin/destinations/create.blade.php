...
@section('content')
    <h1 class="text-2xl text-center font-bold mb-6">Adaugă destinație</h1>
        ...
        <form action="{{ route('admin.destinations.store') }}" method="POST">
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2 mx-4 mt-6">
                <div>
                    ...
                    <input type="text" id="route_tur" name="route_tur" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('route_tur') border-red-500 @enderror" placeholder="Ex:Chișinău" value="{{ old('route_tur') }}" required />
                    @error('route_tur')
